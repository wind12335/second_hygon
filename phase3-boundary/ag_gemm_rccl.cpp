...
int main(int argc, char** argv) {
  ...
  auto run_once = [&]() -> Measurement {
    ...
    for (int i = 0; i < args.q; ++i) {
      const bool has_release = args.path != Path::kGemmOnly &&
                               (args.path != Path::kB0 || i == 0);
      const bool has_gemm = args.path != Path::kCommOnly &&
                            (args.path != Path::kB0 || i == 0);
  ...
  slice_rank_csv << "run_id,rank,rank_count,device,gfx_arch,path,candidate,M,N,K,q,"
  ...
  if (rank == 0) {
    ...
    slice_global_csv << "run_id,rank_count,path,candidate,M,N,K,q,slice_index,slice_bytes,iteration_index,"
                        "t_issue_us,t_release_max_us,t_gemm_start_max_us,t_gemm_end_max_us,"
                        "t_gemm_duration_max_us,correctness_all_ranks,status\n";
  }
  ...
  for (int iter = 0; iter < args.iters; ++iter) {
    ...
    for (int slice_index = 0; slice_index < args.q; ++slice_index) {
      ...
      slice_rank_csv << args.run_id << ',' << rank << ',' << ranks << ',' << device << ",gfx928,"
                     << path_name(args.path) << ',' << args.candidate << ',' << args.m_local << ','
                     << args.n << ',' << args.k << ',' << args.q << ',' << slice_index << ','
                     << slice_bytes << ',' << iter << ",0," << std::fixed << std::setprecision(3)
                     << slice.release_us << ',' << slice.gemm_start_us << ',' << slice.gemm_end_us << ','
      ...
      if (rank == 0) {
        slice_global_csv << args.run_id << ',' << ranks << ',' << path_name(args.path) << ','
                         ...
                         << args.q << ',' << slice_index << ',' << slice_bytes << ',' << iter << ",0,"
                         << std::fixed << std::setprecision(3) << slice_max[0] << ',' << slice_max[1] << ','
                         << slice_max[2] << ',' << slice_max[3] << ',' << (all_pass ? "PASS" : "FAIL")
                         << ',' << (all_pass ? "0" : "1") << '\n';
